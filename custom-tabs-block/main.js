...
function Tabs(args) {
  ...
  var toggle = function() {
    ...
    var resultsElement = document.getElementById("innerBlocks").getElementsByClassName("brand-tab-screen");
    var content = resultsElement[target];
    var toggles = parent.querySelectorAll(settings.trigger);
    var all_content = resultsElement;


    // Update visibility
    for (var i = 0, len = toggles.length; i < len; i++) {
        if(toggles[i] && all_content[i]) {
