...
use vtBlocks\Layouts\Component_Registry;
 ...
 */
function vt_blocks_register_layout_component( array $data ) {
 ...
 */
function vt_blocks_get_layout_component( $type, $key ) {

	if ( empty( $type ) ) {
		return new WP_Error( esc_html__( 'You must supply a type to retrieve a layout component.', 'vt-blocks' ) );
	}

	if ( empty( $key ) ) {
		return new WP_Error( esc_html__( 'You must supply a key to retrieve a layout component.', 'vt-blocks' ) );
	}
 ...
 */
function vt_blocks_get_layouts() {
	$registry = Component_Registry::instance();
 ...
 */
function vt_blocks_get_sections() {
	$registry = Component_Registry::instance();
 ...
 *
 */
function reset_component_registry(){
	$registry = Component_Registry::instance();
