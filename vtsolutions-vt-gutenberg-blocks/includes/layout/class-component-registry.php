...
final class Component_Registry {
	...
	public static function add( array $data ) {
		...
		if ( empty( $data['type'] ) || ! in_array( $data['type'], self::$supported_component_types, true ) ) {
			throw new InvalidArgumentException( esc_html__( 'You must supply a valid component type.', 'vt-blocks' ) );
		}

		if ( empty( $data ) ) {
			throw new InvalidArgumentException( __( 'You must supply valid layout data to register a layout.', 'vt-blocks' ) );
		}
		...
		foreach ( self::$required_data_keys as $required_key ) {
			if ( ! array_key_exists( $required_key, $data ) || empty( $data[ $required_key ] ) ) {
				...
				throw new InvalidArgumentException( sprintf( esc_html__( 'You must supply a %s to register a layout.', 'vt-blocks' ), $required_key ) );
			}
		}
		...
		switch ( $data['type'] ) {
			case 'layout':
				if ( ! empty( self::$layouts[ $data['key'] ] ) ) {
					...
					throw new InvalidArgumentException( sprintf( esc_html__( 'The %s layout is already registered.', 'vt-blocks' ), $data['key'] ) );
				}
			...
			case 'section':
				if ( ! empty( self::$sections[ $data['key'] ] ) ) {
					/* translators: %s: The component's unique key. */
					throw new InvalidArgumentException( sprintf( esc_html__( 'The %s section is already registered.', 'vt-blocks' ), $data['key'] ) );
				}
				self::$sections[ $data['key'] ] = $data;
				break;

			default:
				/* translators: %s: This functions name. Will always be vtBlocks\Layouts\Component_Registry::add(). */
	...
	/**
	 * Reset all the sections and layouts
	 * @return Component_Registry
	 */
