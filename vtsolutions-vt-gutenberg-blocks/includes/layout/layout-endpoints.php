...
const LAYOUT_NAMESPACE = 'vtBlocks/v1';
...
function register_layout_endpoints() {
	...
	register_rest_route(
		...
		[
			...
			'callback'            => function () {
				return new WP_REST_Response( (array) get_user_meta( get_current_user_id(), 'vt_blocks_favorite_layouts', true ) );
			},
	...
	register_rest_route(
		...
		[
			'methods'             => WP_REST_Server::READABLE,
			'callback'            => function () {
				$layouts  = vt_blocks_get_layouts();
				$sections = vt_blocks_get_sections();
	...
	register_rest_route(
		...
		[
			'methods'             => WP_REST_Server::READABLE,
			'callback'            => function () {
				return new WP_REST_Response( (array) vt_blocks_get_layouts() );
			},
			'permission_callback' => function () {
				return current_user_can( 'edit_posts' );
			},
	...
	register_rest_route(
		...
		[
			...
			'callback'            => function ( $request ) {
				...
				return new WP_REST_Response( esc_html__( 'Layout not found.', 'vt-blocks' ) );
			},
			'permission_callback' => function () {
				return current_user_can( 'edit_posts' );
			},
	...
	register_rest_route(
		...
		[
			...
			'callback'            => function ( $request ) {
				...
				$new       = sanitize_key( $body['vt_blocks_favorite_key'] );
				$favorites = (array) get_user_meta( get_current_user_id(), 'vt_blocks_favorite_layouts', true );
				...
				update_user_meta( get_current_user_id(), 'vt_blocks_favorite_layouts', array_values( $favorites ) );
	...
	register_rest_route(
		...
		[
			...
			'callback'            => function ( $request ) {
				...
				$body      = json_decode( $request->get_body(), true );
				$delete_id = sanitize_key( $body['vt_blocks_favorite_key'] );
				$favorites = (array) get_user_meta( get_current_user_id(), 'vt_blocks_favorite_layouts', true );
	...
	register_rest_route(
		...
		[
			'methods'             => WP_REST_Server::READABLE,
			'callback'            => function () {
				return new WP_REST_Response( (array) vt_blocks_get_sections() );
			},
			'permission_callback' => function () {
				return current_user_can( 'edit_posts' );
			},
