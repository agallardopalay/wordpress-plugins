...
export default class Inspector extends Component {
	...
	imageSizeSelect() {
		const getSettings = wp.data.select( 'core/block-editor' ).getEditorSettings();
	...
	render() {
		...
		const postTypeOptions = _.map(postTypes, (postType) => {
			return {
				value: postType.slug,
				label: __( postType.name, 'vt' )
			}
		});

		// Section title tags
		const sectionTags = [
			{ value: 'div', label: __( 'div', 'vt' ) },
			{ value: 'header', label: __( 'header', 'vt' ) },
			{ value: 'section', label: __( 'section', 'vt' ) },
			{ value: 'article', label: __( 'article', 'vt' ) },
			{ value: 'main', label: __( 'main', 'vt' ) },
			{ value: 'aside', label: __( 'aside', 'vt' ) },
			{ value: 'footer', label: __( 'footer', 'vt' ) },
		];
