 ...
 * @return mixed
 */
function vt_render_block_core_latest_posts( $attributes ) {
	...
	if ( $grid_query->have_posts() ) {
		...
		while ( $grid_query->have_posts() ) {
			...
			$post_grid_markup .= sprintf(
				'<div id="post-%1$s" class="%2$s card"><div class="card-body"><div class="row">',
				esc_attr( $post_id ),
			...
			if ( isset( $attributes['displayPostImage'] ) && $attributes['displayPostImage'] && $post_thumb_id ) {
				...
				$post_grid_markup .= sprintf(
					'<div class="ab-block-post-grid-image col-sm-4"><a href="%1$s" rel="bookmark" aria-hidden="true" tabindex="-1">%2$s</a></div>',
			...
			$post_grid_markup .= sprintf(
				'<div class="ab-block-post-grid-text col-sm-8">'
			);
 ...
 */
function vt_register_block_core_latest_posts() {
	...
	register_block_type(
		...
		array(
			...
			'render_callback' => 'vt_render_block_core_latest_posts',
		)
	);
}
add_action( 'init', 'vt_register_block_core_latest_posts' );


/**
 * Create API fields for additional info
 */
function vt_register_rest_fields() {
	/* Add landscape featured image source */
	register_rest_field(
		array( 'post', 'page' ),
		'featured_image_src',
		array(
			'get_callback'    => 'vt_get_image_src_landscape',
	...
	register_rest_field(
		...
		array(
			'get_callback'    => 'vt_get_image_src_square',
			'update_callback' => null,
			'schema'          => null,
		)
	);
	...
	register_rest_field(
		...
		'author_info',
		array(
			'get_callback'    => 'vt_get_author_info',
			'update_callback' => null,
			'schema'          => null,
		)
	);
}
add_action( 'rest_api_init', 'vt_register_rest_fields' );
 ...
 * @return mixed
 */
function vt_get_image_src_landscape( $object, $field_name, $request ) {
 ...
 * @return mixed
 */
function vt_get_image_src_square( $object, $field_name, $request ) {
	$feat_img_array = wp_get_attachment_image_src(
		$object['featured_media'],
 ...
 * @return mixed
 */
function vt_get_author_info( $object, $field_name, $request ) {
	/* Get the author name */
